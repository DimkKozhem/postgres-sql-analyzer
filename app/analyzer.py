...
class SQLAnalyzer:
    ...
    def __init__(self, dsn: Optional[str] = None, mock_mode: bool = False, config: Optional[Dict[str, Any]] = None):
        self.dsn = dsn
    ...
    def analyze_sql(self, sql: str, custom_config: Optional[Dict[str, Any]] = None) -> AnalysisResult:
        ...
        if self.llm_integration and is_valid and explain_json:
            try:
                ai_recommendations = self._get_ai_recommendations(sql, explain_json)
                logger.info(f"Получено {len(ai_recommendations)} AI-рекомендаций")
            except Exception as e:
    ...
    def export_analysis_report(self, result: AnalysisResult, format: str = "json") -> str:
    ...
    def _export_text(self, result: AnalysisResult) -> str:
        ...
        if result.metrics:
            ...
            lines.append("-" * 40)
            lines.append(f"Ожидаемое время выполнения: {result.metrics.estimated_time_ms:.2f} мс")
            lines.append(f"Ожидаемое использование I/O: {result.metrics.estimated_io_mb:.2f} MB")
            lines.append(f"Ожидаемое использование памяти: {result.metrics.estimated_memory_mb:.2f} MB")
            lines.append(f"Ожидаемое количество строк: {result.metrics.estimated_rows:,}")
        ...
        if result.recommendations:
            ...
            if high_recs:
                ...
                for rec in high_recs:
                    ...
                    lines.append(f"    Потенциальное улучшение: {rec.potential_improvement}")
                    lines.append("")
    ...
    async def _get_ai_recommendations(self, sql: str, explain_json: Dict[str, Any]) -> List[Any]:
        ...
        try:
            ...
            confidence_threshold = self.config.get('ai_confidence_threshold', 0.7)
            filtered_recommendations = [
                rec for rec in recommendations 
                if rec.confidence >= confidence_threshold
            ]
            
            logger.info(f"Получено {len(recommendations)} AI-рекомендаций, отфильтровано {len(filtered_recommendations)}")
