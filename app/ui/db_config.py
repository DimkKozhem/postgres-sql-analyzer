...
def _show_llm_analysis(settings: List[Dict[str, Any]]):
    """Отображает LLM анализ конфигурации."""
    ...
    if st.button("🔍 Анализировать конфигурацию", type="primary"):
        with st.spinner("AI анализирует конфигурацию..."):
            try:
                ...
                _display_llm_recommendations(recommendations)
...
def _display_llm_recommendations(recommendations: Dict[str, Any]):
    ...
    has_header = (
        recommendations_text.startswith("## ") or 
        "## 🤖 AI Рекомендации" in recommendations_text or
        "🤖 Рекомендации AI" in recommendations_text or
...
def _parse_structured_text(text: str):
    ...
    for part in parts:
        ...
        if (part.strip() and 
            ...
            not part.startswith('Рекомендации AI') and
            'Приоритет:' in part and  # Должна содержать поле Приоритет
            len(part.strip()) > 50):  # Минимальная длина для валидной рекомендации
...
def _show_mock_db_config():
    ...
    # Mock настройки
    mock_settings = [
        {'name': 'work_mem', 'setting': '4', 'unit': 'MB', 'category': 'Resource Usage', 'short_desc': 'Sets the base maximum amount of memory to be used by a query operation'},
        {'name': 'shared_buffers', 'setting': '128', 'unit': 'MB', 'category': 'Resource Usage', 'short_desc': 'Sets the amount of memory the database server uses for shared memory buffers'},
        {'name': 'effective_cache_size', 'setting': '4', 'unit': 'GB', 'category': 'Query Tuning', 'short_desc': 'Sets the planner\'s assumption about the effective size of the disk cache'},
        {'name': 'max_connections', 'setting': '100', 'unit': '', 'category': 'Connections and Authentication', 'short_desc': 'Sets the maximum number of concurrent connections'}
    ]
