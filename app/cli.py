...
@click.option('--file', '-f', type=click.Path(exists=True), help='Файл с SQL-запросом')
@click.option('--dsn', '-d', help='DSN подключения к PostgreSQL')
@click.option('--mock', is_flag=True, help='Использовать mock режим')
@click.option('--output', '-o', type=click.Path(), help='Файл для сохранения результата')
@click.option('--format', 'output_format', type=click.Choice(['json', 'text']), default='json', help='Формат вывода')
@click.option('--work-mem', type=int, default=4, help='work_mem в MB')
@click.option('--shared-buffers', type=int, default=128, help='shared_buffers в MB')
@click.option('--effective-cache-size', type=int, default=4, help='effective_cache_size в GB')
@click.option('--large-table-threshold', type=int, default=1000000, help='Порог большой таблицы')
@click.option('--expensive-query-threshold', type=float, default=1000.0, help='Порог дорогого запроса')
def analyze(sql: Optional[str], file: Optional[str], dsn: Optional[str], mock: bool, 
    ...
    if file:
        ...
        click.echo("Необходимо указать SQL-запрос через --sql или --file", err=True)
        sys.exit(1)
    
    # Проверяем DSN
    if not mock and not dsn:
        click.echo("Необходимо указать DSN подключения или использовать --mock", err=True)
        sys.exit(1)
    
    try:
        # Создаем анализатор
        analyzer = SQLAnalyzer(dsn if not mock else None, mock)
...
@cli.command()
@click.option('--dsn', '-d', required=True, help='DSN подключения к PostgreSQL')
...
def stats(dsn: str, limit: int, output: Optional[str]):
    ...
    try:
        # Создаем анализатор
        analyzer = SQLAnalyzer(dsn, mock=False)
...
@cli.command()
@click.option('--output', '-o', type=click.Path(), help='Файл для сохранения примеров')
def examples(output: Optional[str]):
    """Показывает примеры тестовых запросов."""
    
    try:
        # Создаем анализатор в mock режиме
...
@click.option('--shared-buffers', type=int, default=128, help='shared_buffers в MB')
@click.option('--effective-cache-size', type=int, default=4, help='effective_cache_size в GB')
def test(dsn: Optional[str], mock: bool, **config_options):
    ...
    try:
        ...
        for i, example in enumerate(examples_data, 1):
            ...
            try:
                ...
                if result.is_valid:
                    ...
                    if result.metrics:
                        click.echo(f"     Время: {result.metrics.estimated_time_ms:.2f} мс")
                        click.echo(f"     I/O: {result.metrics.estimated_io_mb:.2f} MB")
                        click.echo(f"     Рекомендации: {len(result.recommendations)}")
                    passed_tests += 1
                else:
                    click.echo(f"  ❌ Ошибки валидации: {result.validation_errors}")
        ...
        # Итоги тестирования
        click.echo("=" * 50)
        click.echo(f"Результаты тестирования: {passed_tests}/{total_tests} прошли")
